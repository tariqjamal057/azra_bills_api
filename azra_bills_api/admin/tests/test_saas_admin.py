...
from azra_bills_api.admin.models import SAASAdmin
from azra_bills_api.admin.tests.factory import SAASAdminFactory
from azra_bills_api.core.enums import OrderByType
from azra_bills_api.test_utils import (
    assert_database_has,
...
async def test_list_server_error(async_client: AsyncClient, mocker):
    ...
    await mocker("azra_bills_api.admin.views.saas_admin.paginate", side_effect=Exception)
...
async def test_create_succcess(
    ...
    monkeypatch.setattr(
        "azra_bills_api.admin.tasks.saas_admin.send_saas_admin_credentials.delay",
...
async def test_create_server_error(async_client: AsyncClient, mocker, body: dict):
    ...
    """
    await mocker("azra_bills_api.admin.views.saas_admin.SAASAdmin", side_effect=Exception)
    response = await async_client.post(f"{BASE_ROUTE}", json=body)
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
